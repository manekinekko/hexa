...
export const COMMANDS_CACHE: string[] = [];
...
export const DUMP_FILENAME = "hexa.sh";
...
export function updateFile({ filepath, replace, search, override = false }: { filepath: string; replace: string; search?: string; override?: boolean }) {
  let srcContent = (readFileFromDisk(filepath) || "").trim();

  if (search) {
    srcContent = srcContent.replace(search, replace);
  } else {
    srcContent = (override ? [replace] : [srcContent, replace]).join(`\n`);
  }
...
export function dumpCommands() {
  debug(`dumping commands ${chalk.green(JSON.stringify(COMMANDS_CACHE))}`);
  updateFile({
    override: true,
    filepath: DUMP_FILENAME,
    replace: ["# File autogenerated by hexa.run", "#!/usr/bin/env sh", ...COMMANDS_CACHE].join("\n")
  });
}
