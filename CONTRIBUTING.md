...
Do not open issues for general support questions as we want to keep GitHub issues for bug reports and feature requests. You've got much better chances of getting your question answered on [Stack Overflow](https://stackoverflow.com/questions/tagged/project-hexa) where the questions should be tagged with tag `project-hexa`.
...
You can file new issues by filling out our [new issue form](https://github.com/manekinekko/hexa/issues/new).


### <a name="submit-pr"></a> Submitting a Pull Request (PR)
Before you submit your Pull Request (PR) consider the following guidelines:

* Search [GitHub](https://github.com/manekinekko/hexa/pulls) for an open or closed PR
...
* In GitHub, send a pull request to `hexa:master`.
...
```bash
git clone https://github.com/manekinekko/hexa.git
npm install
npm link
```

`npm link` is very similar to `npm install --global` except that instead of downloading the package
from the repo, the just built `build/` folder becomes the global package.
...
Any changes to the files in the `hexa/` folder will immediately affect the global `@manekinekko/hexa` package,
meaning that, in order to quickly test any changes you make to the cli project, you should simply just run `npm run build`
again.

Now you can use `@manekinekko/hexa` via the command line:

```bash
mkdir foo && cd &_
npm link @manekinekko/hexa
hexa init
```

`npm link @manekinekko/hexa` is needed because by default the globally installed `@manekinekko/hexa` just loads
the local `@manekinekko/hexa` from the project which was fetched remotely from npm.
`npm link @manekinekko/hexa` symlinks the global `@manekinekko/hexa` package to the local `@manekinekko/hexa` package.
Now the `hexa` project you cloned before is in two places:
- The folder you cloned it into, npm's folder where it stores global packages.
